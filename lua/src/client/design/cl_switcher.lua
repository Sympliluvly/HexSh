...
surface.CreateFont( "HexSh.sw.25", {
    font = "Arial", 
    extended = false,
    size = ScrH() * 0.025,
    weight = 1000,
} )
surface.CreateFont( "HexSh.sw.20", {
    font = "Arial", 
    extended = false,
    size = ScrH() * 0.020,
    weight = 1000,
...
function PANEL:Init()
	...
	self.ColorInterpolation = HexSh:LerpColor(off_color, off_color, .25);
	...
	self.Label:SetFont("HexSh.sw.25");
...
function PANEL:Paint(w)
	...
	draw.SimpleText(txtt, "HexSh.sw.20", ww, hh, clr, TEXT_ALIGN_LEFT );
...
function PANEL:SetChecked(active, animate)
	...
	if ( animate ) then
		...
		self.SwitchInterpolation = HexSh:Lerp(to, to, .25);

		if (active) then

			self.ColorInterpolation = HexSh:LerpColor(on_color, on_color, .25);
...
function PANEL:SetHelpText(text)
	...
	self.HelpLabel:SetFont("HexSh.VGUI.15");
